  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6 animate-slide-up-fade">
      {(message || error) && (
        <div
          className={`rounded-lg border px-4 py-3 text-sm ${
            message ? 'border-blue-500/30 bg-blue-500/10 text-blue-100' : 'border-red-500/30 bg-red-500/10 text-red-200'
          }`}
        >
          {message || error}
        </div>
      ...
      <div className="text-center mb-4">
        ...
        <button type="button" onClick={onBack} className="text-xs text-primary hover:underline mt-1">
          แก้ไขอีเมล
        </button>
      </div>
      ...
      <div className="text-center">
        <button
          ...
          className={`text-sm flex items-center justify-center gap-2 mx-auto ${
            timer > 0 ? 'text-gray-600 cursor-not-allowed' : 'text-primary hover:text-white cursor-pointer'
          }`}
        >
