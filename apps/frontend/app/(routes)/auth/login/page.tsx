...
function LoginContent() {
  ...
  return (
    <AuthLayout title="ยินดีต้อนรับกลับมา" subtitle="เข้าสู่ระบบเพื่อสานต่อการผจญภัยของคุณ" onClose={handleClose}>
      <form onSubmit={handleSubmit} className="space-y-5">
        {(message || error) && (
          <div
            className={`animate-slide-up-fade rounded-xl border px-4 py-3 text-sm ${
              message ? 'border-green-500/30 bg-green-500/10 text-green-200' : 'border-red-500/30 bg-red-500/10 text-red-200'
            }`}
            role="status"
            aria-live="polite"
          >
        ...
        >
          {isLoading ? (
            <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" aria-label="กำลังเข้าสู่ระบบ" />
          ) : (
            <>
