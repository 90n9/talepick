...
export default function PlayerPage() {
  ...
  const currentSegment: MediaSegment =
    currentNode.segments[currentSegmentIndex] ?? currentNode.segments[0];
  ...
  const unlockedAchievements = SYSTEM_ACHIEVEMENTS.filter(
    (achievement) => achievement.id === 'first_step' || achievement.id === 'completionist'
  );
      ...
      <div className='absolute left-0 top-0 z-[120] flex w-full items-start justify-between p-6'>
        ...
        <div className='flex gap-3'>
          <div className='flex items-center gap-2 rounded-full border border-white/10 bg-black/60 px-4 py-2 backdrop-blur-md cursor-pointer'>
            ...
            />
            <span
              className={`font-mono text-base font-bold ${credits === 0 ? 'text-red-500' : 'text-white'}`}
            >
              {credits}
            </span>
            {nextRefill && (
      ...
      {gameState !== GameState.ENDED && (
        <div className='relative flex h-full w-full flex-col'>
          ...
          >
            {gameState === GameState.PLAYING && (
              <div
                className={`mb-8 transition-all duration-700 ${sceneVisible ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`}
              >
                <div className='rounded-2xl border border-white/10 bg-black/60 p-6 text-gray-100 backdrop-blur-md shadow-2xl'>
                  <p className='font-serif text-lg leading-relaxed md:text-2xl'>
                    {currentSegment.text}
                  </p>
                </div>
              </div>
            ...
            {gameState === GameState.CHOICE && (
              <div className='w-full max-w-2xl mx-auto animate-[slideUpFade_0.6s_ease-out_forwards] px-2 sm:px-4'>
                ...
                <div
                  className={`${selectedChoiceId ? 'pointer-events-none' : ''} flex flex-col gap-4`}
                >
        ...
        <div className='absolute inset-0 z-[140] flex animate-fade-in justify-end bg-black/90 backdrop-blur-md'>
          <div className='flex h-full w-full animate-[slideUpFade_0.5s_ease-out_forwards] flex-col border-l border-white/10 bg-[#1a1d26] shadow-2xl md:w-[480px]'>
            ...
            <div className='custom-scrollbar flex-grow overflow-y-auto p-6'>
              {historyLog.length === 0 && (
                <div className='mt-10 text-center text-gray-500'>ยังไม่มีบันทึกการเดินทาง</div>
              )}
              <div className='space-y-6'>
                {historyLog.map((item, index) => (
                  ...
                  >
                    {item.type === 'choice' && (
                      <div className='absolute -left-[9px] top-0 h-4 w-4 rounded-full border-4 border-[#1a1d26] bg-secondary' />
                    )}
                ...
                <div
                  ref={historyEndRef}
                  className='relative rounded-r-lg border-l-2 border-primary bg-primary/5 pl-4 py-2'
                >
          ...
          >
            <div className='mx-auto max-w-4xl'>
              ...
              >
                ...
                <h1 className='text-2xl font-serif font-bold text-white drop-shadow-2xl md:text-4xl'>
                  {story.title}
                </h1>
              </div>
              ...
              >
                <h3 className='mb-8 text-center text-2xl font-serif font-bold text-white'>
