...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'achievements' | 'history' | 'avatars'>(
    user?.isGuest ? 'history' : 'achievements'
  );
      ...
      <main className='flex min-h-screen items-center justify-center bg-background px-4'>
        <div className='w-full max-w-xl rounded-3xl border border-white/10 bg-surface/70 p-8 text-center shadow-2xl'>
          ...
          <p className='mt-2 text-sm text-gray-400'>
            ติดตามความสำเร็จ ประวัติการเล่น และจัดการอวาตาร์ของคุณ
          </p>
        ...
        <div className='no-scrollbar flex w-full justify-start gap-8 overflow-x-auto px-4 pb-4 md:w-auto md:justify-center'>
          ...
          >
            ...
            {activeTab === 'achievements' && (
              <span className='absolute -bottom-[17px] left-0 h-0.5 w-full bg-accent shadow-[0_0_8px_var(--color-accent)]' />
            )}
          </button>
          <button
          ...
          >
            ...
            {activeTab === 'history' && (
              <span className='absolute -bottom-[17px] left-0 h-0.5 w-full bg-primary shadow-[0_0_8px_var(--color-primary)]' />
            )}
          </button>
          <button
          ...
          >
            ...
            {activeTab === 'avatars' && (
              <span className='absolute -bottom-[17px] left-0 h-0.5 w-full bg-secondary shadow-[0_0_8px_var(--color-secondary)]' />
            )}
          </button>
        </div>
      </div>
              ...
              <>
                ...
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  {SYSTEM_ACHIEVEMENTS.map((achievement) => {
                    ...
                    const creditBonus =
                      'creditBonus' in achievement
                        ? (achievement as { creditBonus?: number }).creditBonus
                        : undefined;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className='flex-grow'>
                          <h4
                            className={`text-base font-bold ${isUnlocked ? 'text-white' : 'text-gray-500'}`}
                          >
                            {achievement.title}
                          </h4>
                ...
                <div className='space-y-4'>
                  {paginatedHistory.map((story, index) => {
                    ...
                    const [textColor, bgColor] = (
                      ENDING_COLORS[details.type] || ENDING_COLORS.default
                    ).split(' ');
                        ...
                        <div className='flex flex-grow flex-col justify-between'>
                          <div>
                            <div className='mb-2 flex items-start justify-between'>
                              <h4 className='text-xl font-bold text-white transition-colors group-hover:text-primary'>
                                {story.title}
                              </h4>
                            </div>
                            ...
                            <div className='relative mb-3 flex items-start gap-3 rounded-lg border border-white/5 bg-black/40 p-3'>
                              ...
                              <div>
                                <span
                                  className={`block text-[10px] font-bold uppercase tracking-wider ${textColor}`}
                                >
                                  {details.type} Ending
                                </span>
              ...
              <>
                <div className='mb-8 flex items-center justify-between'>
                  ...
                  <span className='text-sm text-gray-400'>
                    ปลดล็อก {user.playedStories.length} / {SYSTEM_AVATARS.length}
                  </span>
                </div>
                  ...
                  {SYSTEM_AVATARS.map((avatar) => {
                    ...
                    return (
                      <div key={avatar.id} className='group relative'>
                          ...
                          {!unlocked && (
                            <div className='absolute inset-0 flex items-center justify-center bg-black/90 p-2 text-center opacity-0 transition-opacity group-hover:opacity-100'>
                              <span className='text-[10px] text-gray-300'>
                                {avatar.hint || 'ล็อกอยู่'}
                              </span>
                            </div>
                          )}
                        </div>
                        <p
                          className={`mt-2 truncate text-center text-xs font-medium ${unlocked ? 'text-gray-300' : 'text-gray-600'}`}
                        >
                          {avatar.name}
                        </p>
                        {unlocked && !isEquipped && (
                        ...
                        {isEquipped && (
                          <p className='mt-1 text-center text-[11px] font-semibold text-primary'>
                            กำลังใช้งาน
                          </p>
                        )}
                      </div>
