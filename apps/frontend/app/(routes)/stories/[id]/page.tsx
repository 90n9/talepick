...
export default function StoryDetailPage() {
  ...
  const story = useMemo(
    () => MOCK_STORIES.find((item) => item.id === storyId),
    [storyId],
  );
  ...
  const relatedStories = MOCK_STORIES.filter((item) => item.genre === story.genre && item.id !== story.id).slice(0, 3);
    ...
    <div key={story.id} className="min-h-screen bg-background text-gray-100 animate-fade-in pb-20">
        ...
        <div className="fixed right-4 top-24 z-50 animate-[slideUpFade_0.4s_ease-out_forwards] md:right-8">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">{isToastError ? "กรุณาลองใหม่อีกครั้ง" : "ดำเนินการสำเร็จ"}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative h-[60vh] min-h-[520px] w-full overflow-hidden">
        ...
        <div
          ...
          style={{ transform: `translateY(${scrollY * 0.2}px)`, opacity: 1 - Math.min(1, scrollY / 500) }}
        >
        ...
        <div className="grid grid-cols-1 gap-12 p-6 md:p-12 lg:grid-cols-3">
          <div className="space-y-12 lg:col-span-2">
            <div className="space-y-4">
              <h2 className="border-l-4 border-primary pl-4 text-2xl font-serif font-bold text-white">เรื่องย่อ</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="group relative aspect-video w-full overflow-hidden rounded-xl border border-white/10 bg-black shadow-2xl">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none]">
                  {mediaItems.map((item, idx) => (
                    ...
                    >
                      <Image src={item.thumbnail} alt={`Thumb ${idx}`} fill className="object-cover" sizes="200px" />
                      {item.type === "video" && (
              ...
              <div className="grid grid-cols-2 gap-8 border-t border-white/5 pt-8">
                ...
                <div>
                  <h3 className="mb-2 text-sm uppercase tracking-wider text-gray-500">เขียนบทโดย</h3>
            ...
            <div className="rounded-xl border border-white/5 bg-surface/50 p-6">
              <div className="mb-6 flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">
                  {story.comingSoon ? "-" : `${extras.endings.filter((ending) => ending.unlocked).length}/${extras.endings.length}`}
                </span>
              </div>
              {story.comingSoon ? (
                <div className="py-8 text-center text-sm text-gray-500">ข้อมูลฉากจบจะปรากฏเมื่อเกมเปิดให้เล่น</div>
              ) : (
                  ...
                  {extras.endings.map((ending) => (
                    <div
                      ...
                      className={`flex items-center justify-between rounded-lg border p-3 ${
                        ending.unlocked ? "border-white/10 bg-white/5" : "border-white/5 bg-black/20 opacity-60"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-sm ${ending.unlocked ? "font-medium text-white" : "text-gray-500"}`}>
                      ...
                      {review.adminReply && (
                        <div className="mt-4 ml-4 flex gap-3 rounded-lg border-l-2 border-primary bg-primary/10 p-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm italic text-gray-300">“{review.adminReply.text}”</p>
                          </div>
                        </div>
                      )}
                    </div>
