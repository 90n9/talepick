...
export const GenresPage: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (editingGenre.id) {
      // Update
      setGenres(genres.map(g => g.id === editingGenre.id ? { ...g, ...editingGenre } as Genre : g));
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-sm text-gray-500 mt-1">กำหนดหมวดหมู่ของนิยายเพื่อให้ผู้ใช้ค้นหาได้ง่ายขึ้น</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         ...
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-xl font-bold text-gray-900">{genres.filter(g => g.isActive).length}</h3>
            </div>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
         ...
         <div className="overflow-x-auto flex-1">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ชื่อแนวเรื่อง</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Slug</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">คำอธิบาย</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">จำนวนนิยาย</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedGenres.length > 0 ? paginatedGenres.map((genre) => (
                <tr key={genre.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-bold text-gray-900">{genre.name}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-xs font-mono text-gray-500 bg-gray-100 px-2 py-1 rounded inline-block">{genre.slug}</div>
                  </td>
                  <td className="px-6 py-4 hidden md:table-cell">
                    <div className="text-sm text-gray-500 truncate max-w-xs">{genre.description || '-'}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                     <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${genre.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
           <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-slide-up">
              ...
              <div className="p-6 space-y-4">
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">ชื่อแนวเรื่อง</label>
                    <input 
                       type="text" 
                 ...
                 <div>
                    ...
                    />
                    <p className="text-xs text-gray-400 mt-1">ใช้ภาษาอังกฤษตัวพิมพ์เล็กและขีดกลาง (-) เท่านั้น</p>
                 </div>
