...
import { 
  Flag, Search, Filter, CheckCircle, XCircle, AlertTriangle, 
  ExternalLink, User as UserIcon, Clock, MessageSquare, Eye, BookOpen
} from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-sm text-gray-500 mt-1">จัดการคำร้องเรียนและรายงานความผิดปกติจากผู้ใช้งาน</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-xl font-bold text-gray-900">{reports.filter(r => r.status === ReportStatus.PENDING).length}</h3>
            </div>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-xl font-bold text-gray-900">{reports.filter(r => r.status === ReportStatus.RESOLVED).length}</h3>
            </div>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-xl font-bold text-gray-900">{reports.filter(r => r.targetType === 'review').length}</h3>
            </div>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">เป้าหมาย (Target)</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">นิยาย</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ผู้รายงาน</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">สาเหตุ / วันที่</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedReports.length > 0 ? paginatedReports.map((report) => (
                <tr key={report.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-bold text-gray-900 truncate max-w-[200px]">{report.storyTitle}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                       {new Date(report.timestamp).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: '2-digit', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </td>
               ...
               <div className="p-6 space-y-4">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <div className="flex items-center space-x-2 mb-2">
                         ...
                         <span className={`text-xs px-2 py-0.5 rounded font-medium ${selectedReport.targetType === 'review' ? 'bg-pink-100 text-pink-700' : 'bg-indigo-100 text-indigo-700'}`}>
                      ...
                      {selectedReport.targetType === 'review' && selectedReport.reviewContent && (
                         <div className="mt-3 bg-white p-3 rounded border border-gray-200 shadow-sm relative">
                            ...
                            <p className="text-xs text-gray-500 mb-1">ข้อความที่ถูกรีพอร์ต (โดย {selectedReport.reviewOwnerName})</p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="text-xs font-bold text-gray-500 uppercase block mb-1">ผู้รายงาน</label>
                        <div className="flex items-center">
                     ...
                     <div>
                        <label className="text-xs font-bold text-gray-500 uppercase block mb-1">วันที่แจ้ง</label>
                        <span className="text-sm">{new Date(selectedReport.timestamp).toLocaleString('th-TH')}</span>
                     </div>
                  </div>
