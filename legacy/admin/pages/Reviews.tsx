...
import { 
  Star, Search, Filter, Trash2, Eye, EyeOff, MessageSquare, 
  ThumbsUp, User as UserIcon, AlertTriangle, Check, Trophy, MessageCircle, X
} from 'lucide-react';
...
export const ReviewsPage: React.FC = () => {
  ...
  const lowRatedStories = MOCK_STORIES
    .filter(s => s.rating < 4.0 && s.totalPlayers > 0)
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3">
               {lowRatedStories.length > 0 ? lowRatedStories.map(story => (
                  <div key={story.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg">
                     <div className="flex items-center space-x-2 overflow-hidden">
                        ...
                        <div className="min-w-0">
                           ...
                           <p className="text-[10px] text-gray-500">{story.totalPlayers.toLocaleString()} ผู้เล่น</p>
                        </div>
                     </div>
         ...
         <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 grid grid-cols-2 gap-4">
             ...
             <div className="flex flex-col justify-center items-center bg-gray-100 rounded-lg p-2">
                ...
                <span className="text-2xl font-bold text-gray-700">{reviews.filter(r => r.isHidden).length}</span>
             ...
             <div className="col-span-2 flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-100">
                ...
                <span className="text-lg font-bold text-yellow-700">{reviews.filter(r => r.isFeatured).length}</span>
             </div>
         </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
               <thead className="bg-gray-50">
                  <tr>
                     ...
                     <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/3">ความเห็น</th>
                     <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">วันที่</th>
                     <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">จัดการ</th>
                  </tr>
               </thead>
               <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedReviews.length > 0 ? paginatedReviews.map((review) => (
                     <tr key={review.id} className={`hover:bg-gray-50 group ${review.isHidden ? 'bg-gray-50/50' : ''}`}>
                        ...
                        <td className="px-6 py-4">
                           <p className={`text-sm text-gray-600 line-clamp-2 ${review.isHidden ? 'italic text-gray-400' : ''}`}>
                              {review.comment}
                           </p>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center text-xs text-gray-500">
                           ...
                           <br />
                           {new Date(review.date).toLocaleTimeString('th-TH', {hour: '2-digit', minute:'2-digit'})}
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
           <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
              <div className="flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50">
                 ...
                 <button onClick={() => setReplyModal({isOpen: false, reviewId: null, text: ''})} className="text-gray-400 hover:text-gray-600">
                    <X size={20} />
                 </button>
              </div>
