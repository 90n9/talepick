    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg bg-[#1a1d26] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div className="flex border-b border-white/5 bg-black/10">
            <button
                onClick={() => { setActiveTab('general'); setSuccessMessage(''); }}
            ...
            >
                ...
                {activeTab === 'general' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary shadow-[0_0_8px_var(--color-primary)]" />}
            </button>
            <button
                onClick={() => { setActiveTab('security'); setSuccessMessage(''); }}
                className={`flex-1 py-4 text-sm font-bold transition-colors relative ${
            ...
            >
                ...
                {activeTab === 'security' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-secondary shadow-[0_0_8px_var(--color-secondary)]" />}
            </button>
        </div>
            ...
            {activeTab === 'general' ? (
                <form onSubmit={handleGeneralSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-400 uppercase mb-2">ชื่อที่แสดง (Display Name)</label>
                            <input 
                                type="text"
                                value={name}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                <label className="block text-xs font-bold text-gray-400 uppercase">เลือกรูปโปรไฟล์</label>
                                <button 
                                    type="button"
                            ...
                            {!showCustomUrlInput ? (
                                <div className="grid grid-cols-4 gap-3 bg-black/20 p-3 rounded-xl border border-white/5">
                                    {SYSTEM_AVATARS.map((sysAvatar) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <img src={sysAvatar.src} alt={sysAvatar.name} className="w-full h-full object-cover" />
                                                {!unlocked && (
                                ...
                                <div>
                                    ...
                                    />
                                    <p className="text-xs text-gray-500 mt-1">รองรับลิงก์รูปภาพโดยตรง (JPG, PNG)</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    >
                        {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'บันทึกการเปลี่ยนแปลง'}
                    </button>
                </form>
            ) : (
                <form onSubmit={handlePasswordSubmit} className="space-y-6">
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-400 uppercase mb-2">รหัสผ่านปัจจุบัน</label>
                            <input 
                                type="password"
                                required
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-400 uppercase mb-2">รหัสผ่านใหม่</label>
                            <input 
                                type="password"
                                required
                                minLength={6}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-400 uppercase mb-2">ยืนยันรหัสผ่านใหม่</label>
                            <input 
                                type="password"
                                required
                                minLength={6}
                    ...
                    <button 
                        type="submit" 
                        disabled={isLoading || !currentPassword || !newPassword || newPassword !== confirmPassword || strength < 2}
                    ...
                    >
                         {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'เปลี่ยนรหัสผ่าน'}
                    </button>
                </form>
            )}
        </div>
