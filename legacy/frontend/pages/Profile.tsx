...
import { User as UserIcon, Settings, Heart, Trophy, GitBranch, PlayCircle, Edit3, LogOut, AlertCircle, Zap, Grid, Lock, Calendar, Clock, RotateCcw, Play, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const Profile: React.FC<ProfileProps> = ({ onNavigate, onSelectStory }) => {
  ...
  const [activeTab, setActiveTab] = useState<'achievements' | 'history' | 'avatars'>('achievements');
  ...
  const lastPlayedStoryId = user.playedStories.length > 0 ? user.playedStories[user.playedStories.length - 1] : null;
  const lastPlayedStory = lastPlayedStoryId ? MOCK_STORIES.find(s => s.id === lastPlayedStoryId) : null;
  ...
  const getHistoryDetails = (storyId: string) => {
     const historyData: Record<string, { ending: string; type: 'Good' | 'Bad' | 'Neutral' | 'True' | 'Secret'; date: string }> = {
  ...
  const getEndingColor = (type: string) => {
      switch (type) {
          case 'Good': return 'text-green-400 bg-green-400';
          case 'True': return 'text-accent bg-accent';
          case 'Bad': return 'text-red-400 bg-red-400';
          case 'Secret': return 'text-purple-400 bg-purple-400';
      ...
      <div className="bg-surface/50 p-8 md:p-12 rounded-3xl border border-white/10 animate-fade-in shadow-2xl relative overflow-hidden backdrop-blur-md mb-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            <div className="relative group">
                <div className={`w-32 h-32 rounded-full flex items-center justify-center text-4xl font-bold text-white shadow-[0_0_30px_rgba(168,85,247,0.4)] border-4 border-[#1a1d26] overflow-hidden ${isGuest ? 'bg-gray-700' : 'bg-gradient-to-tr from-primary to-secondary'}`}>
        ...
        <div className="flex md:grid md:grid-cols-4 gap-4 mt-12 relative z-10 overflow-x-auto md:overflow-visible pb-4 md:pb-0 no-scrollbar snap-x">
            <div className="min-w-[140px] md:min-w-0 snap-center p-6 bg-black/40 rounded-2xl border border-white/5 hover:border-primary/30 transition-colors group text-center shrink-0">
                ...
                <div className="text-xs text-gray-400 uppercase tracking-widest whitespace-nowrap">เรื่องที่เล่น</div>
            </div>
            <div className="min-w-[140px] md:min-w-0 snap-center p-6 bg-black/40 rounded-2xl border border-white/5 hover:border-accent/30 transition-colors group text-center shrink-0">
                ...
                <div className="text-3xl font-bold text-white mb-1 flex items-baseline justify-center gap-1">
                    {user.endingsUnlocked} <span className="text-sm text-gray-400 font-normal">/ {totalSystemEndings}</span>
                </div>
                <div className="text-xs text-gray-400 uppercase tracking-widest whitespace-nowrap">ฉากจบที่พบ</div>
            </div>
            <div className="min-w-[140px] md:min-w-0 snap-center p-6 bg-black/40 rounded-2xl border border-white/5 hover:border-secondary/30 transition-colors group text-center shrink-0">
                ...
                <div className="text-xs text-gray-400 uppercase tracking-widest whitespace-nowrap">ความสำเร็จ</div>
            </div>
            <div className="min-w-[140px] md:min-w-0 snap-center p-6 bg-black/40 rounded-2xl border border-white/5 hover:border-pink-500/30 transition-colors group text-center shrink-0">
                ...
                <div className="text-xs text-gray-400 uppercase tracking-widest whitespace-nowrap">รายการโปรด</div>
            </div>
        </div>
      </div>
      ...
      <div className="flex justify-center mb-8 border-b border-white/10 relative">
         <div className="flex gap-8 overflow-x-auto pb-4 px-4 w-full md:w-auto justify-start md:justify-center no-scrollbar">
            ...
            >
                <Trophy size={18} />
                ทำเนียบความสำเร็จ
                {activeTab === 'achievements' && <div className="absolute -bottom-[17px] left-0 w-full h-0.5 bg-accent shadow-[0_0_8px_var(--color-accent)]" />}
            </button>
            <button 
            ...
            >
                <Clock size={18} />
                ประวัติการเล่น
                {activeTab === 'history' && <div className="absolute -bottom-[17px] left-0 w-full h-0.5 bg-primary shadow-[0_0_8px_var(--color-primary)]" />}
            </button>
            <button 
            ...
            >
                <Grid size={18} />
                คลังอวาตาร์
                {activeTab === 'avatars' && <div className="absolute -bottom-[17px] left-0 w-full h-0.5 bg-secondary shadow-[0_0_8px_var(--color-secondary)]" />}
            </button>
         </div>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {SYSTEM_ACHIEVEMENTS.map((ach) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl shadow-inner shrink-0 ${
                                    isUnlocked ? 'bg-black/30' : 'bg-black/50'
                                }`}>
                                    {ach.icon}
                                </div>
                                <div className="flex-grow">
                                    <h4 className={`font-bold text-base ${isUnlocked ? 'text-white' : 'text-gray-500'}`}>
                                        {ach.title}
                                    </h4>
            ...
            <div className="animate-slide-up-fade">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <span className="text-sm text-gray-400">
                        ทั้งหมด {totalHistoryItems} รายการ
                    </span>
                </div>
                    ...
                    <>
                        <div className="space-y-4">
                            {paginatedHistory.map((story, idx) => {
                                ...
                                return (
                                    <div key={`${story?.id}-${idx}`} className="flex flex-col md:flex-row gap-5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl p-5 transition-all hover:scale-[1.01] group">
                                        ...
                                        <div className="flex-grow flex flex-col justify-between">
                                            <div>
                                                <div className="flex justify-between items-start mb-2">
                                                    <h4 className="text-xl font-bold text-white group-hover:text-primary transition-colors">{story?.title}</h4>
                                                </div>

                                                {/* Ending Result Card */}
                                                <div className="bg-black/40 border border-white/5 rounded-lg p-3 flex items-start gap-3 mb-3 relative overflow-hidden">
                                                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${typeColor.split(' ')[1]}`} />
                                                    <div className="mt-1 shrink-0">
                                                        <CheckCircle size={16} className={typeColor.split(' ')[0]} />
                                                    </div>
                                                    <div>
                                                        <span className={`text-[10px] font-bold uppercase tracking-wider block mb-0.5 ${typeColor.split(' ')[0]}`}>
                                                            {details.type} Ending
                                                        </span>
                                            ...
                                            <div className="flex items-center justify-between mt-2 pt-3 border-t border-white/5">
                                                <div className="flex gap-2">
                                                    <span className="text-[10px] border border-white/10 px-2 py-1 rounded text-gray-400 uppercase">{story?.genre}</span>
                                                    <span className="text-[10px] border border-white/10 px-2 py-1 rounded text-gray-400">{story?.duration}</span>
                                                </div>
                                                <button 
                    ...
                    {SYSTEM_AVATARS.map((av) => {
                        ...
                        return (
                            <div key={av.id} className="relative group">
                                <div className={`aspect-square rounded-2xl overflow-hidden border-2 transition-all duration-300 relative ${
                                    unlocked 
                                        ? isEquipped ? 'border-primary shadow-[0_0_15px_var(--color-primary)]' : 'border-white/10 hover:border-white/50'
                                    ...
                                    {!unlocked && (
                                        <div className="absolute inset-0 bg-black/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity p-2 text-center">
                                            <span className="text-[10px] text-gray-300">{(av as any).hint || 'ล็อกอยู่'}</span>
                                        </div>
                                    )}
                                </div>
                                
                                {/* Name Label */}
                                <p className={`text-center text-xs mt-2 font-medium truncate ${unlocked ? 'text-gray-300' : 'text-gray-600'}`}>
                                    {av.name}
                                </p>
                            </div>
                        );
