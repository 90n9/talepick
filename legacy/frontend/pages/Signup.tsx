...
import { Mail, Lock, User, ArrowRight, Check, X as XIcon, Key, ChevronRight, RefreshCw, Edit2, Chrome } from 'lucide-react';
...
export const Signup: React.FC<SignupProps> = ({ onNavigate, onBack }) => {
  ...
  return (
    <AuthLayout 
      ...
      subtitle={step === 'REGISTER' ? "เริ่มต้นการเดินทางของคุณวันนี้" : `กรอกรหัส 6 หลักที่ส่งไปยังอีเมลของคุณ`}
      onClose={onBack}
    >
      {step === 'REGISTER' ? (
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
            <div className="relative group animate-slide-up-fade delay-100 fill-mode-forwards opacity-0">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 transition-all duration-300 group-focus-within:text-primary group-focus-within:scale-110 group-focus-within:drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]" size={18} />
                <input 
                type="text" 
                required
            ...
            <button 
            type="submit"
            disabled={isLoading || strength < 2 || !formData.password || formData.password !== formData.confirmPassword}
