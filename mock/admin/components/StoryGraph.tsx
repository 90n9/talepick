...
export const StoryGraph: React.FC<StoryGraphProps> = ({
  scenes,
  selectedSceneId,
  onSceneSelect,
}) => {
    ...
    scenes.forEach((sourceScene) => {
      sourceScene.choices.forEach((choice) => {
        if (choice.targetSceneId) {
          ...
          if (
            targetScene &&
            sourceScene.x !== undefined &&
            sourceScene.y !== undefined &&
            targetScene.x !== undefined &&
            targetScene.y !== undefined
          ) {
            connections.push({
    ...
    >
      <div className='absolute top-4 right-4 z-10 flex flex-col space-y-2'>
        ...
        <button
          ...
          className='bg-white/90 p-1.5 rounded-md shadow-sm border border-gray-200 hover:bg-gray-50 text-gray-500'
        >
          <Maximize size={16} />
        </button>
      </div>
            ...
            return (
              ...
              >
                ...
                <text
                  ...
                  fontSize='10'
                  fontWeight='bold'
                  fill={scene.isEnding ? '#065F46' : '#374151'}
                >
                  {scene.isEnding
                    ? `ฉากจบ: ${scene.endingType}`
                    : scene.id === 'sc1'
                      ? 'จุดเริ่มต้น'
                      : 'ฉากทั่วไป'}
                </text>
                ...
                <g transform='translate(50, -5)'>
                  {issues.includes('missing_image') && (
                    <ImageOff size={12} className='text-orange-500' x='0' />
                  )}
                  {issues.includes('dead_end') && (
                    <AlertCircle size={12} className='text-red-500' x='-14' />
                  )}
                </g>
              </g>
            );
