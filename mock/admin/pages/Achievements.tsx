...
export const AchievementsPage: React.FC = () => {
  ...
  const [grantUserModal, setGrantUserModal] = useState<{
    isOpen: boolean;
  ...
  const handleSave = () => {
    ...
    if (editingAchievement.id) {
      ...
      setAchievements(
        achievements.map((a) =>
          a.id === editingAchievement.id ? ({ ...a, ...editingAchievement } as Achievement) : a
        )
      );
  ...
  const handleGrantToUser = (userId: string) => {
    ...
    if (user && achievement) {
      addToast(
        `มอบความสำเร็จ "${achievement.title}" ให้กับ ${user.username} เรียบร้อยแล้ว`,
        'success'
      );
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0'>
        <div>
          ...
          <p className='text-sm text-gray-500 mt-1'>
            สร้างภารกิจและรางวัลเพื่อกระตุ้นให้ผู้ใช้งานมีส่วนร่วม
          </p>
        </div>
        <button
            ...
            <div className='relative z-10'>
              ...
              <div className='mb-1 flex items-center space-x-2'>
                ...
                {!achievement.isActive && (
                  <span className='px-2 py-0.5 text-[10px] font-bold rounded bg-gray-200 text-gray-600'>
                    ปิดใช้งาน
                  </span>
                )}
              </div>

              <h3 className='font-bold text-gray-900 text-lg truncate' title={achievement.title}>
                {achievement.title}
              </h3>
              ...
              <div className='mt-6 flex justify-between items-center border-t border-gray-100 pt-4'>
                <div className='flex flex-col'>
                  ...
                  <span className='text-sm font-semibold text-gray-700'>
                    {achievement.usersUnlocked.toLocaleString()} คน
                  </span>
                </div>
                {achievement.unlockTrend !== 0 && (
                  <div
                    className={`flex items-center text-xs font-medium ${achievement.unlockTrend > 0 ? 'text-green-500' : 'text-red-500'}`}
                  >
                    {achievement.unlockTrend > 0 ? (
          ...
          <div className='bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-scale-in'>
            <div className='p-6 text-center'>
              ...
              <p className='text-sm text-gray-500'>
                ...
                <br />
                ผู้ใช้ที่ปลดล็อคไปแล้วจะยังคงมีประวัติอยู่
              </p>
            </div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm'>
          <div className='bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-scale-in'>
            <div className='flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50'>
              <h3 className='font-bold text-gray-900 text-lg'>มอบความสำเร็จให้ผู้ใช้</h3>
              <button
                onClick={() => setGrantUserModal({ isOpen: false, achievement: null })}
            ...
            <div className='p-6'>
              <div className='flex items-center space-x-3 mb-6 bg-indigo-50 p-3 rounded-lg border border-indigo-100'>
                ...
                <div>
                  <p className='text-sm font-bold text-gray-900'>
                    {grantUserModal.achievement.title}
                  </p>
              ...
              <div className='border rounded-lg max-h-48 overflow-y-auto mb-4'>
                {MOCK_USERS.filter(
                  ...
                  <div
                    key={user.id}
                    className='flex justify-between items-center p-3 hover:bg-gray-50 border-b last:border-0 border-gray-100'
                  >
            ...
            <div className='p-6 space-y-4 overflow-y-auto'>
                ...
                <div className='flex gap-2'>
                  {Object.keys(ICON_MAP).map((iconKey) => (
                    <button
                      key={iconKey}
                      onClick={() =>
                        setEditingAchievement({ ...editingAchievement, icon: iconKey })
                      }
                ...
                <div className='flex space-x-4'>
                  <label className='flex items-center space-x-2 cursor-pointer'>
                    <input
                      ...
                      onChange={() =>
                        setEditingAchievement({ ...editingAchievement, type: AchievementType.AUTO })
                      }
              ...
              {editingAchievement.type === AchievementType.AUTO ? (
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEditingAchievement({
                            ...editingAchievement,
                            triggerType: e.target.value as AchievementTriggerType,
                          })
                        }
                ...
                <div className='bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-sm text-yellow-800 flex items-start'>
                  ...
                  <p>
                    ความสำเร็จนี้จะถูกมอบโดยผู้ดูแลระบบผ่านหน้ารายละเอียดผู้ใช้ หรือผ่านกิจกรรมพิเศษ
                  </p>
                </div>
              )}
              ...
              <div className='space-y-3'>
                <h4 className='text-xs font-bold text-gray-500 uppercase mt-2'>
                  รางวัลที่จะได้รับ
                </h4>
                <div className='grid grid-cols-2 gap-4'>
                  <div>
                    <label className='block text-xs font-medium text-gray-700 mb-1'>
                      เครดิตโบนัส (ครั้งเดียว)
                    </label>
                    <div className='relative'>
                      <input
                        ...
                        onChange={(e) =>
                          setEditingAchievement({
                            ...editingAchievement,
                            rewardCredits: parseInt(e.target.value),
                          })
                        }
                    ...
                    <div className='relative'>
                      <input
                        ...
                        onChange={(e) =>
                          setEditingAchievement({
                            ...editingAchievement,
                            rewardMaxCredits: parseInt(e.target.value),
                          })
                        }
                ...
                <div className='bg-purple-50 p-3 rounded-lg border border-purple-100 mt-2'>
                  <label className='block text-xs font-bold text-purple-800 mb-2'>
                    รางวัลอวาตาร์ (Avatar Rewards)
                  </label>

                  {/* List of existing avatars */}
                  {editingAchievement.rewardAvatars &&
                  editingAchievement.rewardAvatars.length > 0 ? (
                      ...
                      {editingAchievement.rewardAvatars.map((url, idx) => (
                        <div key={idx} className='relative group'>
                          <img
                            src={url}
                            alt='Reward'
                            className='w-12 h-12 rounded-full border border-purple-200 object-cover bg-white'
                          />
                          <button
                  ...
                  <p className='text-[10px] text-purple-600 mt-2'>
                    * ผู้เล่นจะได้รับอวาตาร์ทั้งหมดนี้เมื่อปลดล็อคความสำเร็จ
                  </p>
                </div>
              </div>
              ...
              <div className='flex items-center space-x-2 pt-2'>
                <input
                  ...
                  onChange={(e) =>
                    setEditingAchievement({ ...editingAchievement, isActive: e.target.checked })
                  }
        ...
        <div className='fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm'>
          <div className='bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh] animate-scale-in'>
            <div className='p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50'>
              ...
              <button onClick={() => setIsSystemAvatarModalOpen(false)}>
                <X size={20} />
              </button>
            </div>
