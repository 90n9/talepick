...
export const AdminTeamPage: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (editingAdmin.id) {
      ...
      setAdmins(
        admins.map((a) =>
          a.id === editingAdmin.id ? ({ ...a, ...editingAdmin } as AdminAccount) : a
        )
      );
    ...
    setAdmins(
      admins.map((a) => {
        if (a.id === id) {
          ...
          addToast(
            `เปลี่ยนสถานะเป็น ${newStatus === 'active' ? 'ใช้งาน' : 'ระงับ'} เรียบร้อยแล้ว`,
            'info'
          );
  ...
  const getRoleColor = (role: AdminRole) => {
    switch (role) {
      ...
      case AdminRole.STORY_EDITOR:
        return 'bg-blue-100 text-blue-800';
      case AdminRole.USER_MANAGER:
        return 'bg-green-100 text-green-800';
      case AdminRole.ACHIEVEMENT_MANAGER:
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>
        ...
        {activeTab === 'members' && (
          <button
            ...
            className='flex items-center px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 font-medium shadow-sm transition-colors'
          >
        ...
        <div className='p-4 border-b border-gray-100 bg-gray-50 flex items-center'>
          <div className='relative w-full md:w-96'>
            ...
            <input
              type='text'
              placeholder={
                activeTab === 'members' ? 'ค้นหาชื่อ, อีเมล...' : 'ค้นหากิจกรรม, ผู้กระทำ...'
              }
              value={searchTerm}
                ...
                <tbody className='bg-white divide-y divide-gray-200'>
                  {filteredAdmins.map((admin) => (
                    <tr key={admin.id} className='hover:bg-gray-50'>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <span
                          className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getRoleColor(admin.role)}`}
                        >
                          {admin.role}
                        </span>
                      </td>
                      <td className='px-6 py-4 whitespace-nowrap text-center'>
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white transition ${admin.status === 'active' ? 'translate-x-5' : 'translate-x-1'}`}
                          />
                        </button>
                      </td>
                      <td className='px-6 py-4 whitespace-nowrap text-center text-xs text-gray-500'>
                        {new Date(admin.lastActive).toLocaleDateString('th-TH', {
                          day: 'numeric',
                          month: 'short',
                          year: '2-digit',
                          hour: '2-digit',
