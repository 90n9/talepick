...
import { 
  AlertTriangle, ImageOff, GitMerge, FileWarning, CheckCircle, 
  Trash2, RefreshCw, Filter, ExternalLink, Image as ImageIcon
} from 'lucide-react';
...
export const ContentToolsPage: React.FC = () => {
  ...
  const runScan = () => {
    ...
    setTimeout(() => {
      ...
      MOCK_SCENES.forEach(scene => {
        ...
        if (scene.id !== 'sc1' && !MOCK_SCENES.some(s => s.choices?.some(c => c.targetSceneId === scene.id))) {
           foundIssues.push({
  ...
  const getIssueIcon = (type: ContentIssueType) => {
    switch(type) {
        case 'missing_image': return <ImageOff className="text-amber-500" size={18} />;
        case 'dead_end': return <AlertTriangle className="text-red-500" size={18} />;
        case 'orphan': return <GitMerge className="text-blue-500" size={18} />;
        default: return <FileWarning className="text-gray-500" size={18} />;
    }
  };

  const getIssueLabel = (type: ContentIssueType) => {
    switch(type) {
        case 'missing_image': return 'ไม่มีรูปภาพประกอบ (ใน Segment)';
        case 'dead_end': return 'ทางตัน (ไม่มีทางเลือก/ฉากจบ)';
        case 'orphan': return 'ฉากกำพร้า (ไม่มีทางเข้า)';
        default: return 'ข้อผิดพลาด';
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-sm text-gray-500 mt-1">เครื่องมือตรวจหาข้อผิดพลาดของนิยายและจัดการไฟล์ขยะ</p>
        </div>
        <button 
          onClick={runScan} 
        ...
        >
            <RefreshCw size={18} className={`mr-2 ${scanStatus === 'scanning' ? 'animate-spin' : ''}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         ...
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            ...
            <h3 className={`text-2xl font-bold mt-1 ${issues.length > 0 ? 'text-red-600' : 'text-green-600'}`}>{issues.length}</h3>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            <p className="text-xs text-gray-500 uppercase font-semibold">ขาดรูปภาพ</p>
            <h3 className="text-2xl font-bold text-amber-600 mt-1">{issues.filter(i => i.type === 'missing_image').length}</h3>
         </div>
         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                          ...
                          <div key={issue.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                             <div className="flex items-center space-x-4">
                                <div className={`p-2 rounded-lg ${
                                   issue.severity === 'high' ? 'bg-red-100' : (issue.severity === 'medium' ? 'bg-amber-100' : 'bg-blue-100')
                                }`}>
                                   {getIssueIcon(issue.type)}
                                </div>
                                <div>
                                   <div className="flex items-center space-x-2">
                                      <span className="text-sm font-bold text-gray-900">{getIssueLabel(issue.type)}</span>
                                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium uppercase ${
                                         issue.severity === 'high' ? 'bg-red-100 text-red-700' : (issue.severity === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700')
                                      }`}>
                                         {issue.severity}
                                      </span>
                                   </div>
                                   <div className="text-xs text-gray-500 mt-1">
                                      นิยาย: <span className="font-medium text-gray-700">{issue.storyTitle}</span> • 
                                      ฉาก: <span className="font-medium text-gray-700">{issue.sceneTitle}</span> ({issue.sceneId})
                                   </div>
                                </div>
                             </div>
                 ...
                 {unusedAssets.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                       {unusedAssets.map(asset => (
                          <div key={asset.id} className="group relative bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
