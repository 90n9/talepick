...
const StatCard = ({
  ...
  <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-md transition-shadow'>
    ...
    <div className='mt-4 flex items-center justify-between text-sm'>
      {subValue && (
        <span
          className={`${subValue.startsWith('+') ? 'text-green-600' : 'text-red-600'} font-medium`}
        >
          {subValue}
        </span>
      )}
...
const HeatmapCell = ({ intensity }: { intensity: number }) => {
  ...
  return (
    <div
      className={`h-8 w-full rounded ${getColor(intensity)} hover:opacity-80 transition-opacity`}
      title={`ความหนาแน่น: ${intensity}/5`}
    ></div>
  );
};
...
export const Dashboard: React.FC = () => {
  ...
  const topStories = [...MOCK_STORIES].sort((a, b) => b.totalPlayers - a.totalPlayers).slice(0, 5);
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-sm text-gray-500 mt-1'>
            สถิติและข้อมูลการใช้งานประจำวัน{' '}
            ...
            <div className='absolute inset-0 flex items-center justify-center pointer-events-none pb-8'>
              <div className='text-center'>
                <span className='text-2xl font-bold text-gray-900'>
                  {DASHBOARD_STATS.dailyActive.toLocaleString()}
                </span>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100'>
          ...
          <p className='text-xs text-gray-500 mb-6'>
            จำนวนผู้เล่นที่ผ่านแต่ละช่วงของนิยายโดยเฉลี่ย
          </p>
          <div className='h-64'>
            <ResponsiveContainer width='100%' height='100%'>
              <BarChart
                  ...
                  <tr key={story.id} className='hover:bg-gray-50'>
                    <td className='px-6 py-4 whitespace-nowrap'>
                      <div className='flex items-center'>
                        <span
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-3 ${i < 3 ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}
                        >
                          {i + 1}
                        </span>
                        <div>
                    ...
                    <td className='px-6 py-4 text-center text-sm text-gray-600'>
                      {story.totalPlayers.toLocaleString()}
                    </td>
        ...
        <div className='bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col'>
          ...
          <div className='flex-1 overflow-y-auto p-4 space-y-3'>
            {SYSTEM_LOGS.map((log) => (
              ...
              >
                <div
                  className={`mt-0.5 mr-3 flex-shrink-0 ${log.type === 'error' ? 'text-red-500' : log.type === 'warning' ? 'text-amber-500' : 'text-blue-500'}`}
                >
                  {log.type === 'error' ? (
                    <AlertCircle size={16} />
                  ) : log.type === 'warning' ? (
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='flex justify-between items-center mt-1'>
                    <span className='text-xs text-gray-500 flex items-center'>
                      <FileText size={10} className='mr-1' /> {log.location}
                    </span>
