...
import { User, Mail, Lock, Save, Camera, Shield, Key, Image as ImageIcon, Upload, X, Plus } from 'lucide-react';
...
export const ProfilePage: React.FC = () => {
  ...
  const [avatarTab, setAvatarTab] = useState<'default' | 'stories' | 'achievements' | 'users'>('default');
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh] animate-scale-in">
                  ...
                  <div className="p-6 overflow-y-auto">
                      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
                          {(avatarTab === 'default' ? getDefaultAvatars() : 
                            avatarTab === 'stories' ? getStoryAvatars() : 
                            avatarTab === 'achievements' ? getAchievementAvatars() :
                            getUserAvatars()).map((url, idx) => (
                              <button 
                                key={idx} 
                      ...
                      {(avatarTab === 'achievements' && getAchievementAvatars().length === 0) && (
                          <div className="text-center py-8 text-gray-400">ยังไม่มีรางวัลอวาตาร์ในความสำเร็จ</div>
                      )}
