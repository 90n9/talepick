...
import { 
  Settings, Coins, Clock, Zap, Lock, Unlock, 
  Save, AlertTriangle, BatteryCharging, Gift, PlayCircle
} from 'lucide-react';
        ...
        <div className="p-6 bg-gray-50 min-h-[500px]">
           ...
           {activeTab === 'economy' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                 ...
                 <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex items-center space-x-3 mb-6">
                       ...
                       <div>
                          ...
                          <p className="text-xs text-gray-500">กำหนดเพดานเครดิตสูงสุดสำหรับผู้ใช้แต่ละประเภท</p>
                       </div>
                    </div>
                    
                    <div className="space-y-4">
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">สมาชิกทั่วไป (Registered)</label>
                          <div className="flex items-center">
                             <input 
                                type="number" 
                                value={config.creditConfig.baseMaxCreditsNormal}
                                onChange={(e) => handleCreditChange('baseMaxCreditsNormal', parseInt(e.target.value))}
                       ...
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">ผู้เยี่ยมชม (Guest)</label>
                          <div className="flex items-center">
                             <input 
                                type="number" 
                                value={config.creditConfig.baseMaxCreditsGuest}
                                onChange={(e) => handleCreditChange('baseMaxCreditsGuest', parseInt(e.target.value))}
                       ...
                       <div>
                          ...
                          <div className="flex items-center">
                             <input 
                                ...
                                onChange={(e) => handleCreditChange('refillIntervalMinutes', parseInt(e.target.value))}
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none"
                             />
                             <span className="ml-2 text-sm text-gray-500 w-16">นาที</span>
                          </div>
                          <p className="text-xs text-gray-400 mt-1">ผู้ใช้จะได้รับเครดิตเมื่อครบกำหนดเวลานี้</p>
                       </div>
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">จำนวนที่ได้รับ</label>
                          <div className="flex items-center">
                             <input 
                                type="number" 
                                value={config.creditConfig.refillAmount}
                                onChange={(e) => handleCreditChange('refillAmount', parseInt(e.target.value))}
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none"
                             />
                 ...
                 <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="space-y-4">
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">รางวัลการรีวิว (Review Reward)</label>
                          <div className="flex items-center">
                             <input 
                                type="number" 
                                value={config.creditConfig.ratingReward}
                                onChange={(e) => handleCreditChange('ratingReward', parseInt(e.target.value))}
                       ...
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">เครดิตขั้นต่ำในการเล่น</label>
                          <div className="flex items-center">
                             <input 
                                type="number" 
                                value={config.creditConfig.minCreditToPlay}
                                onChange={(e) => handleCreditChange('minCreditToPlay', parseInt(e.target.value))}
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none"
                             />
                             <span className="ml-2 text-sm text-gray-500 w-16">เครดิต</span>
                          </div>
                          <p className="text-xs text-gray-400 mt-1">ผู้ใช้ต้องมีเครดิตอย่างน้อยเท่านี้จึงจะเริ่มอ่านได้</p>
                       </div>
                    </div>
                 </div>

                 {/* Event Mode Card */}
                 <div className={`bg-white p-6 rounded-xl border-2 shadow-sm transition-colors ${config.creditConfig.isEventActive ? 'border-amber-400' : 'border-gray-200'}`}>
                    <div className="flex items-center justify-between mb-6">
                       <div className="flex items-center space-x-3">
                          <div className={`p-2 rounded-lg ${config.creditConfig.isEventActive ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-600'}`}>
                             <Zap size={24} />
                          </div>
                          <div>
                    ...
                    <div className={`space-y-4 ${!config.creditConfig.isEventActive ? 'opacity-50 pointer-events-none' : ''}`}>
                       <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">ตัวคูณรางวัล (Multiplier)</label>
                          <div className="flex items-center">
                             ...
                             <input 
                                ...
                                onChange={(e) => handleCreditChange('eventMultiplier', parseFloat(e.target.value))}
           ...
           {activeTab === 'general' && (
              <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
                 <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    <h3 className="text-lg font-bold text-gray-900 mb-4 border-b pb-2">ข้อมูลเว็บไซต์</h3>
                    <div className="space-y-4">
                       <div>
                 ...
                 <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="flex items-center justify-between">
                       <div>
                          ...
                          <p className="text-sm text-gray-500">ผู้ใช้ทั่วไปจะไม่สามารถเข้าใช้งานได้ (ยกเว้น Admin)</p>
                       </div>
                       <label className="relative inline-flex items-center cursor-pointer">
                          <input 
                             type="checkbox" 
