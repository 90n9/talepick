...
export const StoryEditor: React.FC = () => {
  ...
  const handleDeleteChoice = (choiceId: string) => {
    ...
    handleUpdateScene(
      'choices',
      selectedScene.choices.filter((c) => c.id !== choiceId)
    );
  };
    ...
    <div className='flex flex-col h-[calc(100vh-4rem)] -m-4 md:-m-8'>
      ...
      <div className='h-14 bg-white border-b border-gray-200 px-4 flex items-center justify-between shrink-0 shadow-sm z-30'>
        <div className='flex items-center space-x-3'>
          <NavLink
            to='/stories'
            className='text-gray-500 hover:text-gray-900 p-1.5 hover:bg-gray-100 rounded-md transition-colors'
          >
            <ArrowLeft size={20} />
          </NavLink>
          <div className='border-r border-gray-200 pr-4 mr-2'>
            <h1 className='text-sm font-bold text-gray-900'>
              {currentStory?.title || 'Unknown Story'}
            </h1>
      ...
      <div className='flex-1 flex overflow-hidden'>
        ...
        {viewMode === 'visual' && (
          <>
            ...
            {/* Inspector Panel */}
            <div
              className={`w-[450px] bg-white border-l border-gray-200 flex flex-col overflow-y-auto shadow-xl transform transition-transform duration-300 absolute right-0 bottom-0 top-14 md:relative md:top-0 z-20 ${selectedSceneId ? 'translate-x-0' : 'translate-x-full md:translate-x-0 md:w-0 md:border-l-0'}`}
            >
              {selectedScene ? (
                <div className='flex flex-col h-full'>
                  ...
                  <div className='px-5 py-4 border-b border-gray-100 bg-gray-50/50'>
                    ...
                    <div className='mt-3 flex items-center space-x-2 bg-white px-2 py-1.5 rounded-md border border-gray-200'>
                      ...
                      <button
                        onClick={() =>
                          openAssetSelector('audio', (url) => handleUpdateScene('bgMusic', url))
                        }
                    ...
                    <div className='space-y-3'>
                      <div className='flex justify-between items-center'>
                        <label className='text-xs font-semibold text-gray-500 uppercase flex items-center'>
                          <Layout size={12} className='mr-1.5' /> Segments (
                          {selectedScene.segments?.length || 0})
                        </label>
                        <button
                      ...
                      <div className='space-y-4'>
                        {(selectedScene.segments || []).map((segment, idx) => (
                          <div
                            key={idx}
                            className='bg-white border border-gray-200 rounded-lg p-3 shadow-sm group hover:border-indigo-200 transition-colors'
                          >
                            <div className='flex justify-between items-center mb-2'>
                              <div className='flex items-center space-x-2'>
                              ...
                              <div className='flex-1 space-y-2'>
                                ...
                                <div className='flex items-center space-x-2'>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleUpdateSegment(idx, 'duration', parseInt(e.target.value))
                                    }
                    ...
                    <div className='space-y-3 pt-4 border-t border-gray-100'>
                      <div className='flex justify-between items-center'>
                        <label className='text-xs font-semibold text-gray-500 uppercase'>
                          ทางเลือก ({selectedScene.choices?.length || 0})
                        </label>
                        <button
                      ...
                      <div className='space-y-4'>
                        {selectedScene.choices?.map((choice, idx) => (
                          <div
                            key={choice.id}
                            className='bg-gray-50 p-3 rounded-lg border border-gray-200 group'
                          >
                            <div className='flex justify-between items-start mb-2'>
                              <span className='text-[10px] font-bold text-gray-400 bg-gray-200 px-1.5 rounded'>
                                #{idx + 1}
                              </span>
                              <button
                            ...
                            <div className='flex mt-2 space-x-2'>
                              ...
                              <div className='w-20'>
                                ...
                                <input
                                  ...
                                  value={choice.cost}
                                  onChange={(e) =>
                                    handleUpdateChoice(choice.id, 'cost', parseInt(e.target.value))
                                  }
                        ...
                        <div className='ml-6 mt-2 p-3 bg-green-50 rounded-lg border border-green-100 space-y-3'>
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                handleUpdateScene('endingDescription', e.target.value)
                              }
                              ...
                              <div className='flex-1'>
                                <div className='flex space-x-1'>
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleUpdateScene('endingImage', e.target.value)
                                    }
                                  ...
                                  <button
                                    onClick={() =>
                                      openAssetSelector('image', (url) =>
                                        handleUpdateScene('endingImage', url)
                                      )
                                    }
                ...
                <div className='flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center bg-gray-50/50'>
                  ...
                  <p className='text-sm mt-1 text-gray-500 max-w-[200px]'>
                    คลิกที่โหนดในแผนผังด้านซ้ายเพื่อดูรายละเอียดและแก้ไข
                  </p>
                </div>
              )}
            </div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm'>
          <div className='bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh] animate-scale-in'>
            <div className='flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50'>
              ...
              <button
                onClick={() => setAssetSelector({ ...assetSelector, isOpen: false })}
            ...
            <div className='p-6 overflow-y-auto bg-gray-50'>
              {currentStory?.assets &&
              currentStory.assets.filter((a) => a.type === assetSelector.type).length > 0 ? (
                ...
                <div className='text-center py-12 text-gray-400'>
                  <p>
                    ไม่พบไฟล์ {assetSelector.type === 'image' ? 'รูปภาพ' : 'เสียง'} ในนิยายเรื่องนี้
                  </p>
