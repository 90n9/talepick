...
export const UsersPage: React.FC = () => {
  ...
  const filteredUsers = users.filter((u) => {
    ...
    switch (activeFilter) {
      case 'guest':
        return u.type === UserType.GUEST;
      case 'registered':
        return u.type === UserType.REGISTERED;
      case 'low_credit':
        return u.credits < 10;
      case 'high_achiever':
        return u.achievementsUnlocked >= 5;
      case 'no_rating':
        return u.ratingsCount === 0;
      case 'completed_story':
        return u.storyProgress.some((s) => s.status === 'จบแล้ว');
      default:
        return true;
    }
  });
  ...
  const toggleDonator = (userId: string) => {
    const updatedUsers = users.map((u) =>
      u.id === userId ? { ...u, isDonator: !u.isDonator } : u
    );
    setUsers(updatedUsers);
  ...
  const handleRemoveAvatar = () => {
    setEditForm((prev) => ({
      ...prev,
      avatar: 'https://ui-avatars.com/api/?name=' + encodeURIComponent(editForm.username),
    }));
  };
  ...
  const getProviderIcon = (provider: LoginProvider) => {
    switch (provider) {
      case 'google':
        return <Chrome size={14} className='text-red-500' />;
      case 'email':
    ...
    return (
      <div className='flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden'>
        ...
        <div className='p-5 border-b border-gray-100 flex items-start justify-between bg-gray-50'>
          <div className='flex items-center space-x-4'>
            ...
            <img
              ...
              className='w-16 h-16 rounded-full shadow-sm border-2 border-white object-cover'
              alt='Profile'
            />
            <div>
              ...
              <div className='flex items-center space-x-2 mt-1'>
                <span
                  className={`px-2 py-0.5 rounded text-xs font-medium ${selectedUser.type === UserType.GUEST ? 'bg-gray-200 text-gray-700' : 'bg-blue-100 text-blue-800'}`}
                >
                  {selectedUser.type}
                </span>
                {selectedUser.isDonator && (
        ...
        <div className='flex border-b border-gray-200 bg-white'>
          {(['overview', 'progress', 'achievements', 'ratings', 'history'] as TabType[]).map(
            (tab) => (
              <button
                key={tab}
                ...
                <div className='space-y-6'>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <div className='bg-white p-4 rounded-xl border border-gray-100 shadow-sm'>
                      ...
                      <p className='text-sm font-medium text-gray-900 mt-2'>
                        {new Date(selectedUser.lastActive).toLocaleDateString('th-TH')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className='bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4'>
                    <h3 className='text-sm font-bold text-gray-900 uppercase flex items-center border-b border-gray-100 pb-3'>
                      <Shield size={16} className='mr-2 text-indigo-500' /> ข้อมูลส่วนตัว &
                      ความปลอดภัย
                    </h3>
                  ...
                  <div className='bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6'>
                    ...
                    <div className='border-t border-gray-100 pt-6 space-y-4'>
                      <div className='flex items-center justify-between'>
                        ...
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${selectedUser.isDonator ? 'translate-x-6' : 'translate-x-1'}`}
                          />
                        </button>
                      </div>
                      ...
                      <div className='flex items-center justify-between'>
                        ...
                        <div className='flex items-center space-x-3'>
                          <button
                            onClick={() =>
                              handleMaxCreditChange(selectedUser.id, selectedUser.maxCredits - 10)
                            }
                    ...
                    >
                      <div className='flex justify-between items-start mb-3'>
                        <div>
                          ...
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${prog.status === 'จบแล้ว' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                          >
                            {prog.status}
                          </span>
                        </div>
                      ...
                      <p className='text-xs text-gray-500 text-right'>
                        {prog.progress}% • ล่าสุด{' '}
                        {new Date(prog.lastPlayed).toLocaleDateString('th-TH')}
                      </p>
                    </div>
                  ))
                ) : (
              ...
              <div className='space-y-6'>
                <div className='flex justify-between items-center mb-2 bg-white p-4 rounded-xl border border-gray-200 shadow-sm'>
                  <h3 className='text-sm font-bold text-gray-900'>
                    ความสำเร็จที่ปลดล็อค ({selectedUser.achievements.length})
                  </h3>
                  <button
                ...
                {selectedUser.achievements.length > 0 ? (
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {selectedUser.achievements.map((ach) => (
                      <div
                        key={ach.id}
                        className='bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex justify-between items-center group hover:shadow-md transition-shadow'
                      >
                        <div className='flex items-center space-x-3'>
                        ...
                        <button
                          onClick={() =>
                            handleRemoveAchievementClick(selectedUser.id, ach.id, ach.title)
                          }
                    ...
                    >
                      <div className='flex justify-between items-start'>
                        ...
                        <span className='text-xs text-gray-400'>
                          {new Date(rate.createdAt).toLocaleDateString('th-TH')}
                        </span>
                      </div>
                      <div className='flex items-center my-3'>
                        {[...Array(5)].map((_, i) => (
                          <Star
                            ...
                            size={16}
                            className={
                              i < rate.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'
                            }
                          />
                        ))}
                      </div>
                      ...
                      <div className='mt-4 flex justify-end space-x-3'>
                        <button className='text-xs text-gray-500 hover:text-gray-800 px-2 py-1 hover:bg-gray-100 rounded'>
                          ซ่อนรีวิว
                        </button>
                        <button className='text-xs text-red-500 hover:text-red-700 px-2 py-1 hover:bg-red-50 rounded'>
                          ลบรีวิว
                        </button>
                      </div>
                    </div>
                  ...
                  {DEFAULT_AVATARS.map((url, idx) => (
                    ...
                    >
                      <img
                        src={url}
                        alt='Default'
                        className='w-16 h-16 rounded-full border-2 hover:border-indigo-500'
                      />
                    </button>
                  ))}
                </div>
                  ...
                  >
                    ...
                    <td className='px-6 py-4 whitespace-nowrap text-center hidden sm:table-cell'>
                      <div className='text-xs text-gray-600 space-x-2'>
                        <span title='นิยายที่เล่น'>
                          <BookOpen size={14} className='inline mr-1' />
                          {user.storiesPlayed}
                        </span>
                        <span className='text-gray-300'>|</span>
                        <span title='รีวิวที่เขียน'>
                    ...
                    <td className='px-6 py-4 whitespace-nowrap text-center'>
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.status === UserStatus.ACTIVE ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
                        {user.status}
                      </span>
                    </td>
