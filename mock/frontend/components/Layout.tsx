...
import { Menu, X, User as UserIcon, LogIn, LogOut, ChevronDown, Heart, Settings, Zap, Battery, Edit3 } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col font-sans text-gray-100 bg-background overflow-x-hidden">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-4">
              {isAuthenticated && user ? (
                <>
                  {/* Credit Display */}
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/40 border border-white/5 cursor-pointer" title={`รีฟิล 1 เครดิตทุก 5 นาที (${user.credits}/${user.maxCredits})`}>
                      <Zap size={16} className={`${user.credits > 0 ? 'text-accent' : 'text-red-500'} ${user.credits < user.maxCredits ? 'animate-pulse' : ''}`} fill="currentColor" />
                      <span className={`text-sm font-bold font-mono ${user.credits === 0 ? 'text-red-500' : 'text-white'}`}>
                  ...
                  <div className="relative" ref={dropdownRef}>
                    ...
                    {isUserDropdownOpen && (
                        <div className="absolute right-0 mt-3 w-56 rounded-xl bg-[#1a1d26] border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden animate-scale-in origin-top-right z-50">
                            ...
                            <div className="py-2">
                                <button 
                                    onClick={() => { onNavigate('profile'); setIsUserDropdownOpen(false); }}
                                ...
                                <button 
                                    onClick={() => { onNavigate('wishlist'); setIsUserDropdownOpen(false); }}
                                    className="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:text-white hover:bg-white/5 flex items-center gap-2 transition-colors cursor-pointer"
                                >
                                    <Heart size={16} /> รายการโปรด
                                </button>
                                <button 
                                    className="w-full text-left px-4 py-2.5 text-sm text-gray-300 hover:text-white hover:bg-white/5 flex items-center gap-2 transition-colors opacity-50 cursor-not-allowed"
                                >
               ...
               {isAuthenticated && (
                   <div className="flex items-center gap-2 cursor-pointer transition-opacity hover:opacity-80">
                      <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-black/40 border border-white/5">
                        <Zap size={14} className={`${user && user.credits > 0 ? 'text-accent' : 'text-red-500'}`} fill="currentColor" />
      ...
      <footer className="bg-black py-12 border-t border-white/5 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="col-span-1 md:col-span-2">
            <h2 className="text-xl font-serif font-bold text-white mb-4 flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white font-bold text-xs">C</div>
                {APP_NAME}
            </h2>
            <p className="text-gray-400 max-w-sm">
              เปิดประสบการณ์การเล่าเรื่องแบบ Interactive ที่ทุกทางเลือกของคุณมีความหมาย กำหนดชะตากรรมของคุณเองในโลกแฟนตาซี ไซไฟ และสยองขวัญ
            </p>
          </div>
          <div>
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">แพลตฟอร์ม</h3>
            <ul className="space-y-2">
              <li><button onClick={() => onNavigate('library')} className="text-gray-300 hover:text-primary transition text-sm drop-shadow-md">เลือกดูนิยาย</button></li>
              <li><button onClick={() => onNavigate('support')} className="text-gray-300 hover:text-primary transition text-sm drop-shadow-md">สนับสนุนเรา</button></li>
              <li><button onClick={() => onNavigate('terms')} className="text-gray-300 hover:text-primary transition text-sm drop-shadow-md">ข้อตกลงการใช้งาน</button></li>
              <li><button onClick={() => onNavigate('privacy')} className="text-gray-300 hover:text-primary transition text-sm drop-shadow-md">นโยบายความเป็นส่วนตัว</button></li>
            </ul>
          </div>
          <div>
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">เชื่อมต่อ</h3>
        ...
        <div className="mt-12 text-center text-gray-600 text-sm border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 px-4 sm:px-8 lg:px-12">
          ...
          <span className="text-xs font-mono bg-white/5 px-2 py-1 rounded text-gray-500 hover:text-gray-400 transition-colors cursor-default" title="เวอร์ชันระบบ">
            {APP_VERSION}
          </span>
        </div>
      </footer>
