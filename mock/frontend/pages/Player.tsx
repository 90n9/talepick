...
import { Play, Pause, SkipForward, ArrowLeft, Volume2, VolumeX, ShieldCheck, RotateCcw, Share2, GitBranch, Trophy, Check, Star, CheckCircle, Loader, Zap, AlertTriangle, User as UserIcon, History, X, TrendingUp, Sparkles, Lock } from 'lucide-react';
import { Story, StoryNode, GameState } from '../types';
import { RatingModal } from '../components/RatingModal';
import { StoryCard } from '../components/StoryCard';
import { MOCK_STORIES, REFILL_INTERVAL_MS, SYSTEM_AVATARS, SYSTEM_ACHIEVEMENTS } from '../constants';
...
const MOCK_NODES: Record<string, StoryNode> = {
  ...
  'search': {
    ...
    choices: [
      { id: 'c3', text: 'เก็บคีย์การ์ด', nextNodeId: 'corridor_key' },
    ]
  },
  'door': {
    id: 'door',
  ...
  'fight': {
    ...
    choices: [
      { id: 'end1', text: 'เริ่มใหม่', nextNodeId: 'start' }
    ]
  },
  'game_over_captured': {
    ...
    choices: [
      { id: 'end2', text: 'ลองอีกครั้ง', nextNodeId: 'start' }
    ]
  },
  'escape': {
    id: 'escape',
...
export const Player: React.FC<PlayerProps> = ({ story, onExit, onComplete, onSelectStory }) => {
  ...
  const unlockedAchievements = SYSTEM_ACHIEVEMENTS.filter(a => 
    a.id === 'first_step' || a.id === 'completionist' // Demo: Show these 2 as potential unlocks
  );
    ...
    <div className="fixed inset-0 bg-black z-[100] flex flex-col font-sans text-white">
      ...
      <div className="absolute top-0 left-0 w-full p-6 z-50 flex justify-between items-start pointer-events-none">
        ...
        <div className="flex gap-4 pointer-events-auto">
           ...
           <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-black/60 border border-white/10 backdrop-blur-md">
                <Zap size={18} className={`${user && user.credits > 0 ? 'text-accent' : 'text-red-500'} ${user && user.credits < user?.maxCredits ? 'animate-pulse' : ''}`} fill="currentColor" />
                <span className={`text-base font-bold font-mono ${user && user.credits === 0 ? 'text-red-500' : 'text-white'}`}>
                    {user?.credits}
                </span>
                {nextRefill && (
      ...
      {gameState !== GameState.ENDED && (
        <div className="relative w-full h-full flex flex-col">
          ...
          <div className={`absolute inset-0 z-[60] bg-black flex items-center justify-center transition-opacity duration-700 ease-in-out ${isLoadingNode ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          ...
          <div className="flex-grow relative z-10 flex flex-col justify-end pb-12 px-6 md:px-20 max-w-7xl mx-auto w-full">
            ...
            {gameState === GameState.PLAYING && (
                <div className={`transition-all duration-700 transform ${sceneVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            {gameState === GameState.CHOICE && (
                <div className="animate-slide-up-fade w-full max-w-2xl mx-auto px-4">
                    ...
                    <div className={`flex flex-col gap-4 max-w-xl mx-auto ${selectedChoiceId ? 'pointer-events-none' : ''}`}>
                        {currentNode.choices.map((choice, index) => {
                            // Check achievement requirement
                            const isLocked = choice.requiredAchievement && user && !user.achievements.includes(choice.requiredAchievement);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <h3 className={`text-xl md:text-2xl font-bold mb-2 leading-relaxed transition-colors ${
                                    ...
                                    {isLocked && (
                                        <p className="text-xs text-red-400 mt-2 flex items-center gap-1">
                                            <ShieldCheck size={12} /> ต้องปลดล็อกความสำเร็จ: {choice.requiredAchievement}
                                        </p>
                                    )}
                        ...
                        >
                            <RotateCcw size={14} className="text-gray-400 group-hover:text-white transition-colors" />
                            <span className="text-xs font-bold text-gray-400 group-hover:text-white uppercase tracking-wider">เล่นฉากนี้ซ้ำ</span>
                        </button>
                    </div>
                </div>
            )}
                ...
                <div className="flex-grow overflow-y-auto p-6 custom-scrollbar">
                    ...
                    <div className="space-y-6">
                        {historyLog.map((item, index) => (
                            <div key={index} className={`relative pl-4 border-l-2 ${item.type === 'narrative' ? 'border-gray-700' : 'border-secondary'}`}>
                                {item.type === 'choice' && (
                        ...
                        <div ref={historyEndRef} className="border-l-2 border-primary pl-4 py-2 relative bg-primary/5 rounded-r-lg">
      ...
      {gameState === GameState.ENDED && (
        <div className="absolute inset-0 z-20 overflow-hidden flex flex-col">
            ...
            <div className={`absolute inset-0 z-0 pointer-events-none bg-gradient-to-t from-[#0F1117] via-[#0F1117]/80 to-transparent transition-opacity duration-1000 ${showContent ? 'opacity-100' : 'opacity-0'}`} />
            ...
            >
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className={`text-center mb-8 transition-all duration-700 ease-out delay-100 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    ...
                    <div className={`bg-surface/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 mb-8 shadow-2xl relative overflow-hidden transition-all duration-700 ease-out delay-200 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    ...
                    {/* Simple Stats Badges */}
                    <div className={`flex flex-wrap justify-center gap-4 text-sm text-gray-400 mb-12 transition-all duration-700 ease-out delay-300 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    ...
                    {(unlockedAchievements.length > 0 || unlockedAvatars.length > 0) && (
                         <div className={`mb-16 transition-all duration-700 ease-out delay-500 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {unlockedAchievements.map(ach => (
                                    <div key={ach.id} className="bg-gradient-to-br from-[#1a1d26] to-black border border-white/10 p-5 rounded-2xl flex items-center gap-4 shadow-lg hover:border-accent/50 transition-colors group">
                                        ...
                                        <div className="flex-grow">
                                            <div className="text-xs font-bold text-accent uppercase tracking-widest mb-1">ปลดล็อกความสำเร็จ</div>
                                ...
                                {unlockedAvatars.map(av => (
                                    <div key={av.id} className="bg-gradient-to-br from-[#1a1d26] to-black border border-secondary/30 p-5 rounded-2xl flex items-center gap-4 shadow-[0_0_20px_rgba(168,85,247,0.15)] group relative overflow-hidden">
                                        {/* Glow Effect */}
                                        ...
                                        <div className="w-16 h-16 rounded-full border-2 border-secondary p-0.5 shrink-0 relative z-10">
                                            <img src={av.src} alt={av.name} className="w-full h-full object-cover rounded-full" />
                                        </div>
                                        <div className="flex-grow relative z-10">
                                            <div className="text-xs font-bold text-secondary uppercase tracking-widest mb-1">ได้รับสกินใหม่</div>
                    ...
                    {/* Actions */}
                    <div className={`flex flex-wrap justify-center gap-4 mb-20 transition-all duration-700 ease-out delay-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                        <button 
                            onClick={handleReplay}
                    ...
                    <div className={`border-t border-white/10 pt-16 transition-all duration-700 ease-out delay-1000 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                        <h3 className="text-2xl font-serif font-bold text-white mb-8 text-center">เรื่องราวที่คุณอาจสนใจ</h3>
