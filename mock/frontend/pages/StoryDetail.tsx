...
import { 
  Play, Heart, Star, Clock, GitBranch, ArrowLeft, 
  Share2, AlertCircle, Check, CheckCircle, MessageSquare, Lock, Users, Calendar, Zap, Image as ImageIcon, X, ZoomIn, PlayCircle, Flag, ChevronDown, ChevronUp, CornerDownRight, BadgeCheck
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-gray-100 animate-fade-in pb-20">
      ...
      {showToast && (
        <div className="fixed top-24 right-4 md:right-8 z-50 animate-slide-up-fade">
          <div className={`flex items-center gap-4 backdrop-blur-md border text-white p-4 rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.2)] min-w-[300px] ${
              isToastError 
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
               ...
               {toastMessage.includes('เครดิต') ? <Zap size={24} fill="currentColor" /> : <CheckCircle size={24} />}
            </div>
            <div>
              <h4 className={`font-bold ${isToastError ? 'text-red-400' : 'text-green-400'}`}>{toastMessage}</h4>
      ...
      <div className="max-w-7xl mx-auto relative z-10 bg-background/50 backdrop-blur-sm rounded-t-3xl -mt-6 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 p-6 md:p-12">
            ...
            <div className="space-y-4">
                <h2 className="text-2xl font-serif font-bold text-white border-l-4 border-primary pl-4">เรื่องย่อ</h2>
                <p className="text-gray-300 leading-relaxed text-lg">
                {story.description}
                </p>
                <div className="flex flex-wrap gap-2 mt-4">
                {story.tags.map(tag => (
                    <span key={tag} className="px-3 py-1 bg-surface rounded text-sm text-gray-400 border border-white/5">
                    #{tag}
                    </span>
                ))}
                </div>
                    ...
                    <div className="aspect-video w-full bg-black rounded-xl overflow-hidden relative border border-white/10 shadow-2xl group">
                        {mediaItems[activeMediaIndex]?.type === 'video' ? (
                            <iframe 
                                width="100%" 
                                height="100%" 
                                src={getYouTubeEmbedUrl(mediaItems[activeMediaIndex].src) || mediaItems[activeMediaIndex].src} 
                                title="Trailer" 
                                frameBorder="0" 
                    ...
                    <div className="p-6 rounded-xl bg-gradient-to-br from-blue-900/20 to-purple-900/20 border border-blue-500/30">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-400 text-sm mb-4">เข้าสู่ระบบเพื่อติดตามความคืบหน้า ปลดล็อกความสำเร็จ และบันทึกฉากจบของคุณ</p>
                                <button 
                ...
                <div className="bg-surface/50 rounded-xl p-6 border border-white/5">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <span className="text-xs text-gray-500">
                            {story.comingSoon ? '-' : `${extras.endings.filter(e => e.unlocked).length}/${extras.endings.length}`}
                        </span>
                    </div>
                    {story.comingSoon ? (
                            ...
                            {extras.endings.map((ending) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`text-sm ${ending.unlocked ? 'text-white font-medium' : 'text-gray-500'}`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                         <div className="lg:col-span-2 space-y-4">
                            {reviewsToDisplay.map((review) => (
                                <div key={review.id} className="bg-surface/30 border border-white/5 rounded-xl p-6 hover:bg-surface/50 transition-colors group relative">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="text-xs font-bold text-primary uppercase tracking-wider flex items-center gap-1">
                                                        <BadgeCheck size={14} fill="currentColor" className="text-primary" /> 
                                                        <span className="text-white">ทีมงาน Chronos</span>
                                                    </span>
                                                    <span className="text-[10px] text-gray-500">• {review.adminReply.date}</span>
                                                </div>
                                                <p className="text-sm text-gray-300 italic">"{review.adminReply.text}"</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
